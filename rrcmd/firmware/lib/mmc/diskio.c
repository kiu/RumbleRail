...
#define CS_LOW()	PORTA &= ~(1 << PA2);	/* CS=low */
#define	CS_HIGH()	PORTA |= (1 << PA2);	/* CS=high */

#define SOCKINS		(PINA & (1 << PA1))	/* Card detected.   yes:true, no:false, default:true */
#define SOCKWP		(1)		/* Write protected. yes:true, no:false, default:false */

#define	FCLK_SLOW()	SPCR = (1<<MSTR) | (1<<SPR1) | (1<<SPR0) | (1<<SPE); // clk/64
#define	FCLK_FAST()	SPCR = (1<<MSTR) | (1<<SPE); // clk/2
